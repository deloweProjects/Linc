...
sc_screen_init(struct sc_screen *screen,
...
#ifdef _WIN32
    sc_win_make_frameless(screen->window);
#endif
    ...
    sc_control_bar_init(&screen->control_bar, screen->window);
    screen->display.overlay = &screen->control_bar;
...
sc_screen_handle_event(struct sc_screen *screen, const SDL_Event *event) {
        ...
        case SDL_WINDOWEVENT:
            ...
            switch (event->window.event) {
                ...
                case SDL_WINDOWEVENT_LEAVE:
                    if (screen->control_bar.visible) {
                        screen->control_bar.visible = false;
                        screen->control_bar.hovered = -1;
                        if (screen->has_frame) {
                            sc_screen_render(screen, false);
                        }
                    }
    ...
    if (screen->video && (event->type == SDL_MOUSEMOTION
            || event->type == SDL_MOUSEBUTTONDOWN
            || event->type == SDL_MOUSEBUTTONUP)) {
        int ww = 0, wh = 0, ow = 0, oh = 0;
        SDL_GetWindowSize(screen->window, &ww, &wh);
        SDL_GetRendererOutputSize(screen->display.renderer, &ow, &oh);

        int ex = (event->type == SDL_MOUSEMOTION) ? event->motion.x
                                                  : event->button.x;
        int ey = (event->type == SDL_MOUSEMOTION) ? event->motion.y
                                                  : event->button.y;
        // Scale window points -> drawable pixels to match the overlay draw space
        int mx_px = (ww > 0) ? ex * ow / ww : ex;
        int my_px = (wh > 0) ? ey * oh / wh : ey;

        bool repaint = false;
        bool consumed = sc_control_bar_handle_mouse(&screen->control_bar, event,
                                                    mx_px, my_px, ow, &repaint);
        if (repaint && screen->has_frame) {
            sc_screen_render(screen, false);
        }
        if (consumed) {
            return true;   // do NOT forward to the phone
        }
    }
